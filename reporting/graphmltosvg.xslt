<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg">
  ...
  <xsl:template name="create-node">
    ...
    <xsl:param name="y1">200</xsl:param>
    <!-- some helpers -->
    <xsl:variable name="side" select="1-2*($count mod 2)"/>
    <xsl:variable name="x" select="$level*250"/>
    ...
    <g class="node">
      ...
      <text text-anchor="middle" x="{$x+100}" y="{$y+55}">
        <xsl:value-of select="translate($n/@id,'_',' ')"/>
      </text>
    </g>
    ...
    <xsl:call-template name="query-edge">
      ...
      <xsl:with-param name="x1" select="$x+200"/>
